...
M.setup = function()
    ...
    HWND GetForegroundWindow();
		...
		if vim.loop.os_getenv("WSL_DISTRO_NAME") ~= nil then
			-- wsl
			vim.api.nvim_create_autocmd({ "InsertLeave", "CmdlineLeave" }, {
				callback = function()
					-- local input_status = vim.fn.system("/mnt/d/Application/im-select/im-select-mspy.exe | iconv -f GBK -t UTF-8")
					local input_status = vim.fn.system("/mnt/d/Application/im-select/im-select-mspy.exe")
					if string.find(input_status, "中文") then
						vim.fn.system("/mnt/d/Application/im-select/im-select-mspy.exe  -k=ctrl+space {'英语模式'}")
					end
				end,
			})
		else
